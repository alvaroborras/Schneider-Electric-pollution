...
df_instrument_status = df_instrument[
    ["Measurement date", "Station code", "Item code", "Instrument status"]
].copy()
...
status_cols = {
    f"Instrument status_{p}": f"{p}_status" for p in pollutant_to_code.keys()
}
...
station_locations = (
    df_measure[["Station code", "Latitude", "Longitude"]]
...
def generate_plot(
    ...
    plot_start_date = (
        plot_hist_data.index.min() if not plot_hist_data.empty else pred_start
    )
    if pd.isna(plot_start_date):
        plot_start_date = pred_start

    plt.xlim(
        max(
            plot_start_date - pd.Timedelta(days=15), pred_start - pd.Timedelta(days=30)
        ),
...
for target in targets:
    ...
    logger.info(
        f"  Processing Station {station}, Pollutant {pollutant} (Max Neighbors: {max_neighbors}, Weighted MA) for {pred_start.strftime('%Y-%m')}..."
    )
    ...
    if pd.isna(overall_mean_fallback):
        overall_mean_fallback = 0
    hourly_avg_fallback = hourly_avg_fallback.reindex(
        range(24), fill_value=overall_mean_fallback
    )
    hourly_avg_fallback = hourly_avg_fallback.fillna(0)

    # --- Calculate Historical Monthly Average for Scaling ---
    hist_data_target_monthly = (
        df_measure[
            (df_measure["Station code"] == station)
    ...
    if pd.isna(historical_monthly_avg):
    ...
    neighbor_data_filtered["pollutant_wma3h_centered"] = neighbor_data_filtered.groupby(
        "Station code"
    )[pollutant].transform(
        lambda x: x.rolling(window=3, min_periods=1, center=True).apply(
            weighted_centered_ma, raw=True, kwargs={"weights": neighbor_weights}
        )
    )

    neighbor_data_filtered = neighbor_data_filtered.dropna(
        subset=["pollutant_wma3h_centered"]
    )

    for date in pred_dates:
        neighbors_now_with_ma = neighbor_data_filtered[
            neighbor_data_filtered.index == date
        ].copy()
        weighted_sum = 0.0
        total_weight = 0.0
        ...
        if not neighbors_now_with_ma.empty:
            neighbors_now_with_ma["distance"] = neighbors_now_with_ma[
                "Station code"
            ].apply(lambda s: distances.loc[station, s])

            closest_neighbors = neighbors_now_with_ma.sort_values("distance").head(
                max_neighbors
            )
    ...
    for i, date in enumerate(pred_dates):
        station_predictions[date.strftime("%Y-%m-%d %H:%M:%S")] = (
            scaled_predicted_values[i]
        )  ## Use scaled values
